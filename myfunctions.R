ggpairwise <- function(df, xlab = "", ylab = "", title = "") {
  ...
  my_fn <- function(data, mapping, method = "lm", ...) {
    ...
    df.highlight <- df.highlight[grep("HEATR3", df.highlight$id), ];
    df.highlight$id <- "HEATR3";
    #print(df.highlight);
    ...
    p <- p + geom_text_repel(data = df.highlight, aes(x = x, y = y, label = id), colour = "red", size = 3);
